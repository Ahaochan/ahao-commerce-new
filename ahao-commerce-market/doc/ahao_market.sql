CREATE database if NOT EXISTS `ahao_market` default character set utf8 collate utf8_general_ci;
use `ahao_market`;
...
CREATE TABLE `market_freight_template` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `template_id` varchar(50) COLLATE utf8_bin NOT NULL COMMENT '模板编号',
  `name` varchar(50) COLLATE utf8_bin NOT NULL COMMENT '模板名称',
  `region_id` varchar(50) COLLATE utf8_bin NOT NULL COMMENT '区域编号',
  `shipping_amount` int(10) NOT NULL COMMENT '标准运费',
  `condition_amount` int(10) NOT NULL COMMENT '订单满多少钱则免运费',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
UNIQUE KEY `uk_region_id` (`region_id`) USING BTREE
