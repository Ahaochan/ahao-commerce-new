package moe.ahao.commerce.fulfill.infrastructure.publisher;
    ...
    public void sendMessage(String topic, String message, Integer delayTimeLevel, String type) {
        ...
        try {
            ...
            if (SendStatus.SEND_OK == send.getSendStatus()) {
                ...
                throw FulfillExceptionEnum.SEND_MQ_FAILED.msg(send.getSendStatus().toString());
            }
        } catch (Exception e) {
            log.error("发送MQ消息失败：", e);
            throw FulfillExceptionEnum.SEND_MQ_FAILED.msg(e.getMessage());
        }
    }
